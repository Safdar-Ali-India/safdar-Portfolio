import msalonbeverlyhills from "../assets/msalonbeverlyhills.webp";
import sommetbeauty from "../assets/sommetbeauty.webp";
import independentmed from "../assets/independentmed.webp";
import lorazzo from "../assets/lorazzo.webp";
import drdishadinakar from "../assets/drdishadinakar.webp";

/** Selected agency client work — collapsible section on /projects */
export const clientWork = [
  {
    title: "Lorazzo",
    subtitle:
      "Smart kitchen & bathroom ecommerce for India — hero carousels, category hubs, trust badges, and Shopify storefront tuned for conversions.",
    stack: ["Shopify", "E-commerce"],
    imgLink: lorazzo,
    liveLink: "https://lorazzo.com",
  },
  {
    title: "Dr. Disha Dinakar",
    subtitle:
      "Luxury aesthetic clinic in Dubai — service protocols, treatment listings, booking CTAs, and premium UI for regenerative medicine.",
    stack: ["WordPress", "Custom Theme"],
    imgLink: drdishadinakar,
    liveLink: "https://drdishadinakar.com",
  },
  {
    title: "M Salon Beverly Hills",
    subtitle: "Premium salon site — custom theme, services, gallery, and newsletter modules.",
    stack: ["WordPress", "JavaScript"],
    imgLink: msalonbeverlyhills,
    liveLink: "https://msalonbeverlyhills.com",
  },
  {
    title: "Sommet Beauty",
    subtitle: "Luxury beauty ecommerce — custom cart, lazy-loaded assets, 90+ PageSpeed on mobile.",
    stack: ["Shopify"],
    imgLink: sommetbeauty,
    liveLink: "https://sommetbeauty.com",
  },
  {
    title: "IndependentMed",
    subtitle: "Medical equipment store — WooCommerce catalogue, checkout, and equipment-focused UX.",
    stack: ["WordPress", "WooCommerce"],
    imgLink: independentmed,
    liveLink: "https://independentmed.com",
  },
];
